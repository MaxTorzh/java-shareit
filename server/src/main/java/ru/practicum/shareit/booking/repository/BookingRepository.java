...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM Booking b WHERE b.booker.id = :bookerId " +
            "AND (:state = 'ALL' OR b.status = ru.practicum.shareit.booking.status.BookingStatus.WAITING " +
            "OR b.status = ru.practicum.shareit.booking.status.BookingStatus.REJECTED) " +
            "AND (:state != 'CURRENT' OR (b.start <= :currentTime AND b.end >= :currentTime)) " +
            "AND (:state != 'PAST' OR b.end < :currentTime) " +
            "AND (:state != 'FUTURE' OR b.start > :currentTime) " +
            "AND (:state != 'WAITING' OR b.status = ru.practicum.shareit.booking.status.BookingStatus.WAITING) " +
            "AND (:state != 'REJECTED' OR b.status = ru.practicum.shareit.booking.status.BookingStatus.REJECTED) " +
            "ORDER BY b.start DESC")
    Page<Booking> findByBookerIdAndState(@Param("bookerId") Long bookerId,
    ...
    @Query("SELECT b FROM Booking b JOIN b.item i WHERE i.owner.id = :ownerId " +
            ...
            "AND (:state != 'REJECTED' OR b.status = ru.practicum.shareit.booking.status.BookingStatus.REJECTED) " +
            "ORDER BY b.start DESC")
    Page<Booking> findByItemOwnerIdAndState(@Param("ownerId") Long ownerId,
                                            @Param("state") String state,
                                            @Param("currentTime") LocalDateTime currentTime,
                                            Pageable pageable);
